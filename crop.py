...
def crop_thin(img):
    height = img.size[1]
    width = img.size[0]
    new = img.crop((21,2,(width-3),(height-6)))
    return new
...
if __name__ == '__main__':
    ...
    for path in files:
        out_path_lat = 'lat.png'
	out_path_long = 'long.png'
        if os.path.exists(out_path_lat): continue
	if os.path.exists(out_path_long): continue
        try:
            process_image(path, out_path_lat, out_path_long)
